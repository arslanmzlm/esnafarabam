<header class="border-b border-slate-200 bg-white py-4 shadow">
    <div class="container">
        <div class="flex shrink-0 items-center gap-6">
            ...
            <div id="headerSearch" class="hidden grow lg:block">
                <form method="get" action="{{ route('item.search') }}" class="flex">
                    <input
                        type="search"
                        name="search"
                        placeholder="Taşıt veya ilan numarası ile arama yapın"
                        class="block w-full max-w-115 grow rounded-l border border-slate-200 bg-neutral-100 px-3 py-2 placeholder:text-zinc-500 focus:border-primary focus:outline-none"
                        value="{{ request()->query('search') }}"
                    />

                    <button
                        type="submit"
                        class="rounded-r bg-primary px-3 py-2 text-white transition-opacity hover:opacity-85"
                    >
                        <svg
                            class="size-6"
                            viewBox="0 0 24 24"
            ...
            <div class="ml-auto flex gap-3">
                ...
                @auth
                    <div class="group relative hidden lg:block">
                        <button
                            class="dropdown-button flex items-center gap-4 border border-neutral-100 px-4 py-2 shadow-sm hover:shadow"
                            type="button"
                            data-target="userDropdown"
                        >
                            <span class="hidden text-right lg:block">
                                <span class="block text-sm font-medium text-black dark:text-white">
                                    {{ auth()->user()->profile->full_name ?? 'Kullanıcı' }}
                                </span>
                            </span>
                        </button>
...
<div id="mobileNav" class="fixed inset-0 top-[90px] z-10 space-y-3 bg-white p-4">
    @auth
        <div class="flex bg-neutral-100">
            <a
                href="{{ route('user.profile') }}"
                class="grow py-4 text-center font-medium text-primary"
            >
                ...
                <span class="mt-1 block text-xs text-black">Hesabım</span>
            </a>

            <div class="my-3 w-px bg-neutral-200"></div>
            ...
            <form action="{{ route('logout') }}" method="post" class="shrink-0">
                ...
                <button
                    type="submit"
                    class="flex h-full items-center gap-2 px-6 py-4 text-sm font-medium"
                >
                    <svg
        ...
        @if (auth()->user()->isAdmin())
            <a
                href="{{ route('admin.dashboard') }}"
                class="flex w-full items-center justify-center gap-2 bg-neutral-100 py-3 font-medium"
            >
                <svg
                    class="size-4 fill-current"
                    fill="none"
    ...
    @guest
        <div class="flex bg-neutral-100">
            <a href="{{ route('login') }}" class="grow py-4 text-center font-medium text-primary">
                Giriş Yap
            </a>
            <div class="my-3 w-px bg-neutral-200"></div>
            <a
    ...
    <div class="accordion border-b border-neutral-200 pb-3">
        <button type="button" class="accordion-button flex w-full items-center font-bold">
            Kategoriler
    ...
    <div class="accordion border-b border-neutral-200 pb-3">
        ...
        <div class="accordion-inner mt-2">
            @foreach (\App\Services\PageService::getByCategory(\App\Enums\PageCategory::COMMERCIAL) as $page)
                <a href="{{ route('page.detail', $page->slug) }}" class="block py-1 text-sm">
                    {{ $page->title }}
                </a>
            @endforeach
        </div>
